...
- 配置统一用 **`pydantic-settings`** 的 `BaseSettings` 管理（FastAPI 官方推荐）[$TRAE\_REF](https://fastapi.tiangolo.com/advanced/settings/#use-the-settings)，禁止在业务模块散落 `os.getenv`。
...
**前提**：本仓库是「学习项目 + 生产骨架」的混合体，最大的基建风险是「为将来做准备」而把抽象、分层、依赖做过头。以下为不可逾越的底线，新增任何结构先对照自检。

### 12.1 一句话原则

> 抽象必须服务当下的真实代码；基础设施按真实的触发时机建，不为「看起来更规范」提前预埋。

### 12.2 八条红线

1. **抽象必须有当下的受益者。** 每新增一层（service / repository / 中间件 / 基类 / 装饰器）先回答：现在有没有调用方？没有受益者的空壳就是为抽象付费（如尚无业务时的空 `app/services/`）。
2. **推迟到触发时机，而非提前预埋。** 详见「12.3 推迟清单」：基础设施按真实需求点再建。
3. **单一职责，但不为细分而细分。** 一个文件承载一个完整职责是允许的；按「读的人会不会迷路」判断是否拆分，而非按行数或「内聚感」。
4. **分层方向是硬约束，层数是软约束。** 依赖单向（`api → agents → tools → storage`）必须遵守；各层内部是否再分子层，交由实际复杂度决定。
5. **全局机制给一次、做对。** 统一响应、SSE 协议、异常转码、trace_id 等横切机制每个只实现一次，后续一律复用、禁止绕行。临时造轮子（手拼 SSE 帧、散落 `os.getenv`）是漏不是省。
6. **依赖与配置从简起步。** 新增依赖必须有当下真实用途；新增配置项 = `Settings` 加字段 + `.env.example` 补一行。
7. **扩展点「留位置」，不要「做实现」。** 预留好位置（如 SSE 已定义 `tool_call / tool_result / ping` 事件）但不预写将返工的实现。
8. **怀疑自己的编辑器视野。** 「加一层更规范」的冲动，未必是让此时读者更清楚，可能只是满足外部「最佳实践」印象。

### 12.3 本仓库推迟清单（照此执行，暂不实现）

| 结构 | 触发时机（再开始建） |
| ---- | ------------------------------------------------------------ |
| Alembic 迁移 | 第一张真实业务表出现时 |
| `/readyz` 就绪探针 | 存储/外部依赖接入后 |
| API Key 鉴权 Depends | 服务对外开放时 |
| 会话态持久化（Redis） | 多 worker 真正需要共享状态时 |
| RAG 相关超出 import 的深层封装 | 阶段四 RAG 落地时 |
| service / repository 层 | 出现明确的跨用例业务复用点时 |
| K8s manifests / CI 流水线 | 部署到集群前 |

### 12.4 自查三问（新增抽象前）

1. 这个抽象现在有没有至少一个真实调用方？
2. 把它推迟一个月，会有多少返工成本？（< 半天 → 推迟）
3. 去掉它，谁会读不清代码？

三问全过再动手；有一问存疑，先写下意图、引入真实触发时机后再实现。
